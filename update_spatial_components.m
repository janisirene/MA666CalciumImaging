...
if nargin < 2 || (isempty(A_) && isempty(C))  % at least either spatial or temporal components should be provided
    ...
    if ~isempty(C); K = size(C,1); elseif islogical(A_); K = size(A_,2); else K = size(A_,2) - options.nb; end
end
...
if nargin < 4 || isempty(A_); 
    ...
    if islogical(A_)     % check if search locations have been provided, otherwise estimate them
        IND = double(A_)/diag(sum(double(A_)));
...
if strcmpi(options.spatial_method,'constrained');
    if spatial_parallel         % solve BPDN problem for each pixel
        Nthr = max(20*maxNumCompThreads,round(d*Ts/2^24));
        ...
        for nthr = 1:Nthr     
            ...
            parfor px = 1:siz_row(nthr)
                ...
                if ~isempty(ind);
                    ...
                    [~, ~, a, ~] = lars_regression_noise(Ytemp(px,fn)', Cf(ind2,fn)', 1, sn_temp(px)^2*Ts);
        ...
        for px = 1:d   % estimate spatial components
            ...
            if ~isempty(ind);
                ind2 = [ind,K+(1:size(f,1))];
                [~, ~, a, ~] = lars_regression_noise(Y(px,fn)', Cf(ind2,fn)', 1, options.sn(px)^2*Ts);
                A(px,ind2) = a';
                sA(px) = sum(a);
            end
            if show_sum
...
elseif strcmpi(options.spatial_method,'regularized')
    [A,C] = update_spatial_lasso(Y, A_, Cf, IND, options.sn, [], options.max_iter_lasso, options);
...
C = max(imresize(C, [size(C, 1), Ts*tsub]),0);
if T ~= Ts*tsub
    C = padarray(C, [0, T-Ts*tsub], 'post');
end
